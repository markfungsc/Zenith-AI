...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'diary' | 'nutrition' | 'planner' | 'analysis' | 'me' | 'schedule' | 'store'>('dashboard');
  ...
  if (isInitialLoad) {
    return (
      <div className="h-screen bg-slate-950 flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-slate-500 font-black uppercase tracking-widest text-sm">Initializing Protocol...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 overflow-hidden">
      <header className="px-6 py-4 flex justify-between items-center border-b border-slate-800 bg-slate-900/50 backdrop-blur-md sticky top-0 z-50">
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className={`w-10 h-10 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 border-2 transition-all ${activeTab === 'me' ? 'border-white scale-110' : 'border-transparent'}`} 
          />
        </div>
      </header>
        ...
        <div className="max-w-6xl mx-auto p-4 md:p-8">
          {profile ? renderContent() : (
             <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-slate-500 font-black uppercase text-xs tracking-widest">Constructing Profile...</p>
             </div>
          )}
        </div>
      </main>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-slate-900/90 backdrop-blur-xl border-t border-slate-800 p-3 z-50 md:static md:bg-transparent md:border-t-0 md:p-6 md:flex md:justify-center">
        <div className="flex justify-between items-center w-full max-w-2xl md:gap-4 bg-slate-950 md:bg-slate-900/50 p-2 rounded-[2rem] border border-slate-800 shadow-2xl overflow-x-auto no-scrollbar">
          ...
          <NavButton active={activeTab === 'store'} onClick={() => setActiveTab('store')} icon={<ShoppingBag size={18} />} label="Store" />
        </div>
      </nav>
    </div>
  );
...
const NavButton = ({ active, onClick, icon, label }: any) => (
  <button onClick={onClick} className={`flex flex-col items-center gap-1 min-w-[56px] py-2 rounded-2xl transition-all duration-300 ${active ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20 scale-105' : 'text-slate-500 hover:text-slate-300'}`}>
    {icon}
