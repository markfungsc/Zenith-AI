...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    'dashboard' | 'diary' | 'nutrition' | 'planner' | 'analysis' | 'me' | 'schedule' | 'store'
  >('dashboard');
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'diary':
        return (
          <WorkoutLog
            ...
            prefill={pendingWorkout}
            onClearPrefill={() => setPendingWorkout(null)}
          />
        );
      case 'schedule':
        return <CalendarView workouts={workouts} meals={meals} />;
      case 'nutrition':
        return <NutritionTracker meals={meals} onUpdateMeals={saveMeals} />;
      case 'planner':
        return (
          <WorkoutPlanner
            profile={profile}
      ...
      case 'analysis':
        return <AnalysisView workouts={workouts} profile={profile} />;
      case 'me':
        return <MeProfile profile={profile} onUpdateProfile={updateProfile} />;
      case 'store':
        return <Store />;
      default:
        return <Dashboard workouts={workouts} meals={meals} profile={profile} />;
...
const NavButton = ({ active, onClick, icon, label }: any) => (
  <button
    onClick={onClick}
    className={`flex flex-col items-center gap-1 min-w-[56px] py-2 rounded-2xl transition-all duration-300 ${active ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20 scale-105' : 'text-slate-500 hover:text-slate-300'}`}
  >
    {icon}
