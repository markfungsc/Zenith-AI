...
import { ShoppingBag, Star, CheckCircle, CreditCard, ShieldCheck, Zap, X, Loader2, Sparkles, Smartphone, QrCode } from 'lucide-react';
...
const PROGRAMS: Program[] = [
  {
    ...
    price: 499,
    duration: '4 Weeks',
    features: ['Custom Macro Protocol', 'Daily Video Guidance', 'Weak Point Analysis', 'Access to Elite Group'],
  ...
  },
  {
    ...
    price: 899,
    duration: '12 Weeks',
    features: ['RPE-Based Programming', 'Form Review Feedback', 'Competition Peaking', '1-on-1 Strategy Call'],
  ...
  },
  {
    ...
    price: 649,
    duration: '8 Weeks',
    features: ['VO2 Max Development', 'Hypertrophy Cycles', 'Mobility Maintenance', 'Nutrition Automation'],
  ...
  return (
    <div className="space-y-12 pb-12 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h2 className="text-4xl font-black text-white uppercase tracking-tighter">Elite Programs</h2>
          <p className="text-slate-400 mt-2 font-medium">Transform your physique with world-class protocols.</p>
        </div>
        <div className="flex items-center gap-4 bg-slate-900/50 p-4 rounded-3xl border border-slate-800">
          <ShieldCheck className="text-indigo-400" />
          <p className="text-xs font-bold text-slate-300 uppercase tracking-widest">Secure Global Payments</p>
        </div>
      </div>
          ...
          >
            ...
            <div className="p-8 flex-1 flex flex-col">
              <div className="flex justify-between items-center mb-4">
                <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest bg-indigo-500/10 px-3 py-1 rounded-full">Pro Program</span>
              ...
              <div className="mt-auto pt-6 border-t border-slate-800 flex items-center justify-between">
                <div>
                   <p className="text-[10px] font-black text-slate-500 uppercase tracking-tighter">Investment</p>
          ...
          <div className="bg-slate-900 border border-slate-800 w-full max-w-xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            {paymentStatus === 'success' ? (
              <div className="p-12 text-center space-y-6">
                ...
                <h3 className="text-3xl font-black text-white uppercase tracking-tighter">Payment Successful</h3>
                <p className="text-slate-400 font-medium">Your program <strong>{selectedProgram.name}</strong> is now unlocked in your 'Plans' tab.</p>
                <button 
              ...
              <>
                <div className="p-8 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
                   <div className="flex items-center gap-4">
                     ...
                     <div>
                       <h4 className="font-black text-white text-lg uppercase tracking-tight">Secure Checkout</h4>
                       <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Order ID: #{Math.floor(Math.random() * 900000)}</p>
                     </div>
                   </div>
                   <button onClick={closeCheckout} className="p-2 text-slate-500 hover:text-white transition-colors"><X /></button>
                </div>
                ...
                <div className="p-8 space-y-8">
                  ...
                  <div className="bg-slate-800/40 p-5 rounded-2xl flex justify-between items-center border border-slate-700/50">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xl font-black text-indigo-400">HKD ${selectedProgram.price}</p>
                    </div>
                  </div>
                  ...
                  <div className="min-h-[220px]">
                    {paymentMethod === 'card' && (
                      <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
                        <div className="space-y-1">
                          ...
                          <input type="text" placeholder="•••• •••• •••• ••••" className="w-full bg-slate-800 border border-slate-700 rounded-xl px-5 py-4 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono" />
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Expiry</label>
                            <input type="text" placeholder="MM/YY" className="w-full bg-slate-800 border border-slate-700 rounded-xl px-5 py-4 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono" />
                          </div>
                          <div className="space-y-1">
                    ...
                    {paymentMethod === 'payme' && (
                      <div className="flex flex-col items-center justify-center text-center space-y-4 py-6 animate-in slide-in-from-right-4 duration-300">
                        ...
                        <div>
                          <p className="text-white font-black text-xl uppercase tracking-tighter">Pay with HSBC PayMe</p>
                          <p className="text-slate-500 text-sm mt-1">Open PayMe on your phone to authorize.</p>
                        </div>
                      </div>
                    )}
                      ...
                      <div className="flex flex-col items-center justify-center text-center space-y-4 py-6 animate-in slide-in-from-right-4 duration-300">
                        <div className="w-32 h-32 bg-white rounded-2xl flex items-center justify-center p-2 shadow-xl shadow-blue-500/10">
                           <div className="w-full h-full bg-slate-100 rounded-xl flex flex-col items-center justify-center gap-1 border-2 border-dashed border-slate-300">
                              ...
                              <span className="text-[8px] font-black text-slate-400">SCAN TO PAY</span>
                           </div>
                        </div>
                        <div>
                          <p className="text-white font-black text-xl uppercase tracking-tighter">Alipay / WeChat Pay</p>
                          <p className="text-slate-500 text-sm mt-1">Scan the QR code with your wallet app.</p>
                        </div>
                      </div>
                    )}
                  </div>
