    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-slate-400">
          Personalized programs based on your PRs & Goal:{' '}
          <span className="text-indigo-400 font-bold">{profile.goal}</span>
        </p>
      </div>
        ...
        <div className="bg-slate-900/80 border-2 border-indigo-500/30 p-8 rounded-[2.5rem] space-y-6 animate-in slide-in-from-top-4">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between border-b border-slate-800 pb-6">
            <div className="flex-1 w-full">
              <label className="text-[10px] text-slate-500 font-black uppercase mb-1 block">
                Plan Name
              </label>
              <input
            ...
            <div className="flex gap-2 w-full md:w-auto self-end">
              <button
                onClick={handleSave}
                className="flex-1 md:flex-none bg-emerald-600 px-6 py-2 rounded-xl font-bold text-white flex items-center gap-2 justify-center hover:bg-emerald-500 transition-colors"
              >
                <Save size={18} /> Save
              </button>
              <button
                onClick={() => setTempPlan(null)}
                className="flex-1 md:flex-none bg-slate-800 px-6 py-2 rounded-xl font-bold text-slate-400 justify-center"
              >
                Discard
              </button>
            </div>
          ...
          <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
            {tempPlan.map((day, i) => (
              <div key={i} className="bg-slate-800/20 rounded-2xl p-6 border border-slate-700/50">
                <h4 className="font-black text-indigo-400 mb-4 uppercase tracking-widest">
                  {day.day}
                </h4>
                <div className="grid grid-cols-1 gap-3">
                  {day.exercises.map((ex, j) => (
                    <div
                      key={j}
                      className="flex flex-col md:flex-row md:items-center justify-between p-4 bg-slate-900/50 rounded-xl border border-slate-800"
                    >
                      ...
                      <div className="flex gap-4 items-center">
                        <div className="text-right">
                          ...
                          <p className="text-[10px] text-slate-500 font-bold uppercase">
                            {ex.sets} sets x {ex.suggestedReps || ex.reps} reps
                          </p>
                        </div>
                        {ex.notes && (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {savedPlans.map(plan => (
              <div
                key={plan.id}
                className="bg-slate-900 border border-slate-800 rounded-[2rem] overflow-hidden flex flex-col hover:border-indigo-500/40 transition-all group shadow-lg"
              >
                <div className="p-6 pb-4">
                  ...
                  <div className="space-y-4">
                    {plan.routines.map((day, i) => (
                      <div
                        key={i}
                        className="bg-slate-800/40 rounded-2xl p-4 border border-slate-700/50 group/day hover:border-indigo-500/30 transition-all"
                      >
                        ...
                        <div className="space-y-2">
                          {day.exercises.slice(0, 3).map((ex, j) => (
                            <div key={j} className="flex justify-between text-[10px] font-bold">
                              ...
                              <span className="text-indigo-400/80 whitespace-nowrap">
                                {ex.suggestedWeight}
                              </span>
                            </div>
                          ))}
