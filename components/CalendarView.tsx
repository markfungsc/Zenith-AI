      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 font-medium">
            Viewing {viewMode === 'month' ? `${monthName} ${year}` : 'Current Week'}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 bg-slate-900 p-1 rounded-2xl border border-slate-800">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-black uppercase transition-all ${viewMode === 'month' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
          >
            Month
          </button>
          <button
            onClick={() => setViewMode('week')}
            className={`px-4 py-2 rounded-xl text-xs font-black uppercase transition-all ${viewMode === 'week' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
          >
            Week
          </button>
        </div>
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-[2.5rem] p-8 shadow-2xl">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-xl font-black text-white">
                {monthName} <span className="text-slate-600">{year}</span>
              </h3>
            ...
            <div className="grid grid-cols-7 gap-2">
              {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
                <div
                  key={d}
                  className="text-center text-[10px] font-black text-slate-600 uppercase mb-2"
                >
                  {d}
                </div>
              ))}
          ...
          <div className="bg-slate-900 border border-indigo-500/20 rounded-[2.5rem] p-8 shadow-2xl h-full sticky top-8">
            ...
            {selectedDayData && selectedDayData.workouts.length > 0 ? (
              <div className="space-y-6">
                {selectedDayData.workouts.map(w => (
                  <div key={w.id} className="space-y-4">
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-1 h-4 rounded-full ${w.type === 'strength' ? 'bg-indigo-500' : w.type === 'cardio' ? 'bg-emerald-500' : 'bg-purple-500'}`}
                      />
                      <h5 className="font-black text-slate-200 uppercase text-xs tracking-widest">
                        {w.title}
                      </h5>
                    </div>
                    <div className="space-y-2 pl-3">
                      {w.exercises?.map(ex => (
                        <div
                          key={ex.id}
                          className="flex justify-between items-center bg-slate-800/40 p-3 rounded-xl border border-slate-800/50"
                        >
                      ...
                      {w.cardio?.map(act => (
                        ...
                        >
                          <span className="text-xs font-bold text-slate-400">
                            {act.type} ({act.intensity})
                          </span>
                          <span className="text-xs font-black text-emerald-300">
                            {act.duration}m | {act.distance}km
                          </span>
                        </div>
                      ))}
                    </div>
                ...
                <div className="pt-6 border-t border-slate-800">
                  ...
                  <p className="text-[10px] text-slate-600 mt-4 text-center italic">
                    Calorie tracking syncs with your food log.
                  </p>
                </div>
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center py-20 text-center opacity-40">
                ...
                <p className="text-xs text-slate-600 mt-1 max-w-[150px]">
