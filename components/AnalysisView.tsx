  ...
  if (workouts.length < 1) {
    return (
      <div className="text-center py-20 bg-slate-900/20 rounded-3xl border-2 border-dashed border-slate-800">
        ...
        <p className="text-slate-600 mt-2">AI needs data to detect imbalances and suggest improvements.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-white uppercase tracking-tight">Personalized Coaching</h2>
          <p className="text-slate-400">Deep insights for <span className="text-indigo-400 font-bold">{profile.name}</span></p>
        </div>
        <button 
          onClick={runAnalysis}
          disabled={loading}
      ...
      {loading ? (
        <div className="bg-slate-900/50 p-16 rounded-[2.5rem] border border-indigo-500/20 flex flex-col items-center justify-center gap-6 shadow-2xl">
          <div className="relative">
            ...
            <Sparkles className="absolute -top-2 -right-2 text-yellow-500 animate-pulse" size={28} />
          </div>
          <div className="text-center">
            <p className="text-2xl font-black text-white uppercase tracking-widest">Evaluating Bio-metrics</p>
            <p className="text-slate-400 mt-2">Gemini is reviewing your PRs and training volume...</p>
          </div>
        </div>
      ) : analysis && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in slide-in-from-bottom-8 duration-700">
          ...
          <div className="md:col-span-2 bg-gradient-to-br from-indigo-900/20 to-purple-900/20 border border-indigo-500/30 p-10 rounded-[2.5rem] flex flex-col md:flex-row items-center gap-10 shadow-2xl">
            ...
            <div className="flex-1 text-center md:text-left">
              <h3 className="text-3xl font-black text-white mb-4 uppercase tracking-tighter">Consistency & Peak Power</h3>
              <p className="text-slate-400 leading-relaxed text-lg">Your trajectory towards <strong>{profile.goal}</strong> is strong. Based on your weight/rep progressions, your neural adaptation is peaking. You are training more efficiently than {Math.round(analysis.progressScore)}% of users with similar PR profiles.</p>
            </div>
          </div>
          ...
          <div className="bg-slate-900/50 p-8 rounded-[2rem] border border-emerald-500/20 space-y-6 shadow-lg">
            ...
            <div className="space-y-4">
              {analysis.recommendations.map((item, i) => (
                <div key={i} className="flex items-start gap-4 p-5 bg-emerald-500/5 rounded-2xl border border-emerald-500/10 hover:bg-emerald-500/10 transition-colors">
